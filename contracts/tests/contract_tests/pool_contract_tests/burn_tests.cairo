...
fn test_burn_position_within_price_range() {
    ...
    assert(
        liquidity_after == (liquidity_before - burn_amount.into()), 'Incorrect liquidity amount',
    );
}


#[test]
fn test_burn_position_narrow_price_range() {
    let (params, expected_amount0, expected_amount1) =
        TestParamsImpl::narrow_range_burn_test_values();
    let (pool_address, manager_address, _token0, _token1) = setup_test_environment(params);
    ...
    let liquidity_before = pool_dispatcher.get_liquidity();
    assert(liquidity_before == 5670207847624059387904, 'Initial liquidity should be ');
...
#[test]
fn test_burn_position_wide_price_range() {
    let (params, expected_amount0, expected_amount1) =
        TestParamsImpl::wide_range_burn_test_values();
    let (pool_address, manager_address, _token0, _token1) = setup_test_environment(params);
    ...
    let liquidity_before = pool_dispatcher.get_liquidity();
    assert(liquidity_before == 5670207847624059387904, 'liquidity should not be 0');
...
impl TestParamsImpl of TestParamsT {
    ...
    fn in_range_burn_test_values() -> (TestParams, u256, u256) {
    ...
    fn narrow_range_burn_test_values() -> (TestParams, u256, u256) {
        let params = TestParams {
    ...
    fn wide_range_burn_test_values() -> (TestParams, u256, u256) {
        let params = TestParams {
